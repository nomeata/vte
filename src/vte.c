...
static void
vte_view_clear_primary_cb(GtkClipboard *clipboard, gpointer owner)
{
	VteView *terminal;
	terminal = owner;
	if (terminal->pvt->has_selection) {
...
static void
vte_view_copy_primary_cb(GtkClipboard *clipboard, GtkSelectionData *data,
...
/* Note that the clipboard has cleared. */
static void
vte_view_clear_clipboard_cb(GtkClipboard *clipboard, gpointer owner)
{
	VteView *terminal;
	terminal = owner;
...
/* Supply the selected text to the clipboard. */
static void
vte_view_copy_clipboard_cb(GtkClipboard *clipboard, GtkSelectionData *data,
		     guint info, gpointer owner)
{
	VteView *terminal;
	terminal = owner;
 ...
 */
static gchar *
vte_view_cellattr_to_html(VteView *terminal, const VteCellAttr *attr, const gchar *text) {
	GString *string;
	guint fore, back;
...
vte_terminal_char_to_cell_attr(VteView *terminal, VteCharAttributes *attr)
{
	const VteCell *cell;

	cell = vte_screen_find_charcell(terminal->pvt->buffer->pvt->screen, attr->column, attr->row);
	if (cell)
		return &cell->attr;
	return NULL;
}
 ...
 */
char *
vte_view_attributes_to_html(VteView *terminal, const gchar *text, GArray *attrs) {
	GString *string;
	guint from,to;
	const VteCellAttr *attr;
	char *escaped, *marked;
	...
	while (text[from] != '\0') {
		...
		if (text[from] == '\n') {
			...
			marked = vte_view_cellattr_to_html(terminal, attr, escaped);
...
vte_view_real_copy_clipboard(VteView *terminal)
{
	...
	clipboard = gtk_widget_get_clipboard(&terminal->widget, GDK_SELECTION_CLIPBOARD);
	...
	if (terminal->pvt->selection_clipboard != NULL) {
		...
		gtk_clipboard_set_with_owner(clipboard,
					     targets,
					     n_targets,
					     vte_view_copy_clipboard_cb,
					     vte_view_clear_clipboard_cb,
					     G_OBJECT(terminal));
 ...
 * @terminal: a #VteView
 *
 * Copies the selected text in @terminal to PRIMARY
 */
static void
vte_view_real_copy_primary(VteView *terminal)
{
	...
	if (terminal->pvt->selection != NULL) {
		...
		gtk_clipboard_set_with_owner(clipboard,
					     targets,
					     n_targets,
					     vte_view_copy_primary_cb,
					     vte_view_clear_primary_cb,
					     G_OBJECT(terminal));
		gtk_clipboard_set_can_store(clipboard, NULL, 0);
	}
}
